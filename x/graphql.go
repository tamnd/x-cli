...
type GraphQL struct {
	...
	// healed are the feature flags X has added since defaultFeatures was last
	// refreshed, learned from its own rejections. See features.go.
	mu     sync.Mutex
...
func (g *GraphQL) features() string {
	blob := defaultFeatures
	if g.cfg.Features != "" {
		blob = g.cfg.Features
	}
	g.mu.Lock()
	defer g.mu.Unlock()
	return withFeatures(blob, g.healed)
}
...
func (g *GraphQL) get(ctx context.Context, op string, variables map[string]any) ([]byte, string, error) {
	...
	// so a stale cached token self-heals instead of surfacing as needs-auth. A
	// missing feature flag heals the same way, also once. Both are inside the
	// loop because both change the request.
	var minted, healed bool
	for {
		...
		if !healed {
			if names := missingFeatures(err); len(names) > 0 && g.heal(names) {
				healed = true
				g.cfg.verbosef("X now requires %s on %s; sending it as false, fold it into defaultFeatures",
					strings.Join(names, ", "), op)
				continue
			}
		}
